  ...
  module ApplicationHelper
    def flash_messages
      ...
      flash.map do |type, message|
        ...
        tag.div(message, class: "alert-box #{type} margin-top")
      end.join.html_safe
    end
    ...
    def sortable(klass, column)
      ...
      label = safe_join([t("#{klass.to_s.tableize}.#{column}", default: column.titleize), tag.span])
      link_to label, send(path, order: "#{column}_#{direction}"), class: css_class
    end

    def extract_sort_info
      return unless params[:order].to_s =~ /^([\w_.]+)_(desc|asc)$/
