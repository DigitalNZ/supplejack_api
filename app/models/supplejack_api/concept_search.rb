...
module SupplejackApi
    ...
    def facet_list
      ...
      @facet_list.keep_if {|f| Concept.valid_facets.include?(f) }
      @facet_list
    end
  
    def field_list
      return @field_list if @field_list
      valid_fields = ConceptSchema.fields.keys.dup
    ...
    def group_list
      ...
      @group_list.keep_if {|f| Concept.valid_groups.include?(f) }
      @group_list
    end
    ...
    def sort
      ...
      begin
        field = Sunspot::Setup.for(Concept).field(value)
        return value
