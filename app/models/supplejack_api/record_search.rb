...
module SupplejackApi
    ...
    def field_list
      ...
      valid_fields = RecordSchema.fields.keys.dup
    ...
    def self.role_collection_restrictions(scope)
      ...
      if scope
        ...
        if RecordSchema.roles[role].record_restrictions
          restrictions = RecordSchema.roles[role].record_restrictions
        end
      end
    ...
    def search_builder
      @search_builder ||= Sunspot.new_search(Record) do
        ...
        RecordSearch.role_collection_restrictions(options[:scope]).each do |field, values|
