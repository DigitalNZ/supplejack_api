...
Given(/^these records:$/) do |table|
  table.hashes.each do |hash|
    ...
    record = create(:record, internal_identifier: "abc:#{rand(1000..10000)}", record_id: hash[:record_id])
    fragment = build(:record_fragment, hash.except('record_id'))
...
Given /^a record$/ do
  ...
  @record.fragments << build(:record_fragment)
  @record.save
end
