  ...
  describe ConceptsController, type: :controller do
    ...
    describe 'GET show' do
      ...
      it 'renders a error when records is not found' do
        allow(Concept).to receive(:custom_find).and_raise(
          Mongoid::Errors::DocumentNotFound.new(Concept, ['123'], ['123'])
        )
    ...
    describe 'GET index' do
      ...
      it 'renders a error when the requested field doesn\'t exist' do
        allow(SearchSerializer).to receive(:new).and_raise(
          Sunspot::UnrecognizedFieldError.new('No field configured for Concept with name "something"')
        )
