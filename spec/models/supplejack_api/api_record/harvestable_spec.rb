  ...
  describe ApiRecord::Harvestable do
    ...
    describe "#create_or_update_fragment" do
      ...
      context "existing fragment" do
        let!(:fragment) { record.fragments.create(source_id: 'nz_census', name: 'John Smith', age: 50) }
    ...
    describe "#set_status" do
      let!(:fragment) { record.fragments.build(source_id: 'nz_census', name: 'John Smith') }
    ...
    describe "#clear_attributes" do
      let(:record) { FactoryGirl.create(:record, landing_url: "http://goo.com/sitemap.xml") }
      ...
      it "nullifies the landing_url" do
        record.clear_attributes
        record.landing_url.should be_nil
      end
    end
