  ...
  describe '#spawn' do
    ...
    context 'without params, defaults to all metrics collected before today' do
      ...
      it 'generates per day collection metrics' do
        ...
        expect(todays_collection_metric.records_added_to_user_sets).to eq record_metrics_yesterday.sum(&:added_to_user_sets)
        expect(todays_collection_metric.records_added_to_user_stories).to eq record_metrics_yesterday.sum(&:added_to_user_stories)
        expect(todays_collection_metric.total_views).to eq record_metrics_yesterday.sum(&:page_views) + record_metrics_yesterday.sum(&:appeared_in_searches) + record_metrics_yesterday.sum(&:user_set_views) + record_metrics_yesterday.sum(&:user_story_views)
      end
      ...
      it 'generates all collection metrics' do
        ...
        expect(all_metrics.total_views).to eq (yesterday_record_metrics).sum(&:page_views) + (yesterday_record_metrics).sum(&:appeared_in_searches) + (yesterday_record_metrics).sum(&:user_set_views) + (yesterday_record_metrics).sum(&:user_story_views)
      end
    ...
    context 'with date_range, it only creates collection metrics for the given date range' do
      ...
      it 'does not delete old metrics when it is appending new data' do
        current_appeared_in_searches = SupplejackApi::RecordMetric.where(date: Time.now.utc.tomorrow).sum(:appeared_in_searches)
        ...
        expect(more_collection_metric.searches).to eq SupplejackApi::RecordMetric.where(date: Time.now.utc.tomorrow).sum(:appeared_in_searches) + current_appeared_in_searches
