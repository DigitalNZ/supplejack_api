...
RSpec.describe SupplejackApi::RecordMetric do
  ...
  describe '::spawn' do
    let!(:record_metric) do
      create(:record_metric, record_id: 1, page_views: 1, display_collection: 'NDHA', date: Time.current.yesterday)
    end
    ...
    it 'increments an existing Record Metric by a given hash of metrics' do
      SupplejackApi::RecordMetric.spawn(
        record_metric.record_id, { 'page_views' => 7, 'appeared_in_searches' => 6, 'user_set_views' => 10 }, 'NDHA'
      )
