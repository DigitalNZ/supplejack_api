...
RSpec.describe 'Story Items Endpoints', type: :request do
  ...
  describe '#index' do
    context 'when requesting without a user_key' do
      before { get "/v3/stories/#{story.id}/items.json?api_key=#{api_key}" }
    ...
    context 'when requesting with a invalid user_key' do
      before { get "/v3/stories/#{story.id}/items.json?api_key=#{api_key}&user_key=fakeuserkey" }
    ...
    context 'when requesting with a invalid story id' do
      before { get "/v3/stories/thisstoryiddontexist/items.json?api_key=#{api_key}&user_key=#{story.user.api_key}" }
    ...
    context 'when requesting with a user_key' do
      before { get "/v3/stories/#{story.id}/items.json?api_key=#{api_key}&user_key=#{story.user.api_key}" }
  ...
  describe '#show' do
    ...
    context 'when item exists' do
      before do
        get "/v3/stories/#{story.id}/items/#{item.id}.json?api_key=#{api_key}&user_key=#{story.user.api_key}"
      end
    ...
    context 'when story dosent exist' do
      before do
        get "/v3/stories/thisstoryiddontexist/items/#{item.id}.json?api_key=#{api_key}&user_key=#{story.user.api_key}"
      end
    ...
    context 'when story item dosent exist' do
      before { get "/v3/stories/#{story.id}/items/fakeitemid.json?api_key=#{api_key}&user_key=#{story.user.api_key}" }

      it 'returns error' do
        response_attributes = JSON.parse(response.body)

        expect(response_attributes).to eq(
          { 'errors' => I18n.t('errors.story_item_not_found', id: 'fakeitemid', story_id: story.id) }
        )
      end
    end
  end
  ...
  describe '#destroy' do
    ...
    context 'when story id does not exist' do
      before { delete "/v3/stories/fakestoryid/items/#{item.id}.json?api_key=#{api_key}&user_key=#{story.user.api_key}" }
    ...
    context 'when story item dosent exist' do
      before { delete "/v3/stories/#{story.id}/items/fakeitemid.json?api_key=#{api_key}&user_key=#{story.user.api_key}" }
    ...
    context 'when story & item exists' do
      before { delete "/v3/stories/#{story.id}/items/#{item.id}.json?api_key=#{api_key}&user_key=#{story.user.api_key}" }
