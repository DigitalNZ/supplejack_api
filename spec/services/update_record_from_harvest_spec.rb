...
RSpec.describe UpdateRecordFromHarvest do
  ...
  describe '#call' do
    let(:payload) do
      ...
      "description"=>
       ["Exemplar record should only have 2 attachments (Attachment A and Attachment B)"],
    ...
    context 'record already exists' do
      let!(:current_record_fragment) do
        build(:fragment,
          ...
          content_partner: ['current_content_partner', 'current_content_partner_two'],
        )
      end

      let!(:current_record) { create(:record, internal_identifier: "end-to-end_exemplar_TEST_record-20190306T1140", fragments: [current_record_fragment]) }
